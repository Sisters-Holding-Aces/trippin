...
import { Button, Card, Text, Dialog, Portal, PaperProvider, TextInput, Modal } from 
"react-native-paper";
     ...
     <ScrollView style={styles.container}>
       ...
       {holiday?.title?<Card>
         <Card.Content style={styles.holiday_card}>
           ...
           <Text 
variant="bodyMedium">{holiday.startDate.toDate().toLocaleDateString()}</Text>
           ...
           >
             <Text style={{ color: "darkslateblue", fontWeight: "bold" }}>See 
memories...</Text>
           </Pressable>
         </Card.Content>
         <Card.Actions>
       ...
       {holiday ? <Portal>
         ...
         >
           ...
           <Dialog.Content>
             <TextInput label="Title" value={newHolidayTitle} onChangeText={(text) => 
setNewHolidayTitle(text)} />
             <TextInput label="Info" value={newHolidayInfo} onChangeText={(text) => 
setNewHolidayInfo(text)} />
           </Dialog.Content>
           <Dialog.Actions>
             <Button
               onPress={() => {             
                 const promise1 = editHoliday(userId, holiday.id, "title", 
newHolidayTitle);
                 const promise2 = editHoliday(userId, holiday.id, "info", 
newHolidayInfo);
         ...
         >
           ...
           <Dialog.Content>
             <Text variant="bodyMedium">
               By deleting this holiday, you will delete all the associated memories 
as well.
             </Text>
           </Dialog.Content>
           <Dialog.Actions>
                 ...
                 return (
                   <Card key={memory.id}>
                     <Card.Content>
                       ...
                       <Text 
variant="bodyMedium">{memory.date.toDate().toLocaleDateString()}</Text>
         ...
         >
           <Dialog.Title>Edit</Dialog.Title>
           <Dialog.Content>
             <TextInput label="Title" value={newMemoryTitle} onChangeText={(text) => 
setNewMemoryTitle(text)} />
             <TextInput label="Info" value={newMemoryNote} onChangeText={(text) => 
setNewMemoryNote(text)} />
           </Dialog.Content>
           <Dialog.Actions>
             <Button
               onPress={() => {
                 const promise1 = editMemory(userId, holiday.id, memoryToBeEdit.id, 
"title", newMemoryTitle);
                 const promise2 = editMemory(userId, holiday.id, memoryToBeEdit.id, 
"note", newMemoryNote);
                 Promise.all([promise1, promise2]).then(() => {
                   memoriesByHoliday(userId, holiday.id)
                     .then((res) => {
         ...
         >
           <Dialog.Title>Alert</Dialog.Title>
           <Dialog.Content>
             <Text variant="bodyMedium">Are you sure you want to delete this memory?
</Text>
           </Dialog.Content>
           <Dialog.Actions>
             <Button
